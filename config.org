...
#+BEGIN_SRC emacs-lisp
  ...
  (defun sk-add-c-header-declaration ()
    ...
    (save-excursion
      ...
      (let* ((str (filter-buffer-substring (pos-bol) (pos-eol)))
  ...
  (defun get-github-latest-release-info (owner repo)
    (json-parse-string
     (with-temp-buffer
       (url-insert-file-contents
        (format "https://api.github.com/repos/%s/%s/releases/latest"
                owner repo))
       (buffer-string))))

  (defun get-github-latest-release-version (owner repo &optional release-info)
    (gethash "tag_name"
             (or release-info
                 (get-github-latest-release-info owner repo))))

  (defun download-github-release-package (owner repo ver name target)
    (url-copy-file
     (format "https://github.com/%s/%s/releases/download/%s/%s"
             owner repo ver name)
     target))
  ...
  (defun string-truncate-right (string length &optional append)
    "If STRING is longer than LENGTH, return a truncated version.
  When truncating, and APPEND is not nil, APPEND is always appended to the string,
  so the resulting string may be longer than the original if LENGTH is smaller
  than the length of APPEND."
    (let ((strlen (length string)))
      (if (<= strlen length)
          string
        (setq length (max 0 (- length (length append))))
        (concat (substring string 0 length) append))))
#+END_SRC
     ...
     '("%e"
       (:eval
        (let* ((evil-info (and (featurep 'evil) evil-mode
               ...
               (left-info (concat evil-info buffer-info buffer-name mode-and-vc))
               (right-info (concat input-name line-info pos-info))
               (left-len (string-width left-info))
               (right-len (string-width right-info))
               (left-info
                (if (> (window-width) (+ 2 left-len right-len))
                    left-info
                  (string-truncate-right
                   left-info (- (window-width) right-len 2))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package inf-elixir
    ...
    (evil-leader/set-key-for-mode 'inf-elixir-mode
      "l" 'my-comint-history))

  (use-package elixir-mode
    :config
    (defun install-latest-elixir-ls ()
      (interactive)
      (let* ((target-dir (expand-file-name "elixir-ls/" user-emacs-directory))
             (ver-file (expand-file-name "VERSION" target-dir))
             (owner "elixir-lsp")
             (repo "elixir-ls")
             (info (get-github-latest-release-info owner repo))
             (ver (get-github-latest-release-version owner repo info))
             (zip (format "/tmp/%s.zip" (make-temp-name "elixir-ls-"))))
        (if (and (file-exists-p ver-file)
                 (string-equal ver (file-to-string ver-file)))
            (message "Latest Elixir LSP(%s) is already installed." ver)
          (save-window-excursion
            (delete-directory target-dir t)
            (download-github-release-package
             owner repo ver (concat "elixir-ls-" ver ".zip") zip)
            (shell-command (format "unzip %s -d %s" zip target-dir))
            (write-region ver nil ver-file)
            (delete-file zip)
            (message "Elixir LSP(%s) is installed." ver)))))
