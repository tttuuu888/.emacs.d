...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package package
    ...
    (defun sk-package-upgrade-all ()
      ...
      (let* ((vc-pkgs (my-vc-packages))
             (vc-upgradable (my-upgradable-vc-packages))
             (vc-upgraded (my-upgrade-vc-packages vc-upgradable))
             (upgradeable-tmp (package--upgradeable-packages))
             (upgradeable (seq-remove (lambda (pkg) (memq pkg vc-pkgs))
                                      upgradeable-tmp))
             (upgraded (cl-remove-if-not #'package-upgrade upgradeable))
             (upgraded-all (append upgradeable vc-upgradable))
             (pkg-cnt (length upgraded-all)))
        (if (not upgraded-all)
            (message "No packages to upgrade")
          (message (format "%s packages upgraded. %s" pkg-cnt upgraded-all))))))
  ...
  (use-package package-vc
    ...
    (cl-defun my-upgrade-vc-packages
        (&optional (vc-pkgs (my-upgradable-vc-packages)))
