...
(use-package vertico
  ...
  (defun my-vertico-clear-input ()
    "Clear minibuffer and insert '#' if the input started with it."
    (interactive)
...
Otherwise truncate via ORIG with MAX."
  ...
  (defun +vertico-kill-ring-page-scroll (orig)
    "Page-jump scrolling for multiline kill-ring candidates for side window."
    (if (and (eq (vertico--metadata-get 'category) 'kill-ring)
             (overlayp vertico--candidates-ov)
             (window-live-p (overlay-get vertico--candidates-ov 'window)))
        (let* ((win (overlay-get vertico--candidates-ov 'window))
          ...
          (when (or (< i s) (>= i (+ s page)))   ; left page -> recenter
            (setq s (+vertico-kill-ring--center i avail)
                  page (+vertico-kill-ring--fit s avail)))
          ;; +1 renders the next candidate to fill the bottom gap.
...
(use-package consult
  ...
  (defun consult-rg-here ()
    (interactive)
    (consult-ripgrep default-directory (+consult-search-initial)))

  (defun sk-consult-git-grep ()
    (interactive)
    (consult-git-grep nil (+consult-search-initial)))
  ...
  (defun consult-occur ()
    (interactive)
    (sk-consult-line (+consult-search-initial)))
...
(use-package consult-yasnippet
  ...
  (defun my-consult-yasnippet--preview (orig-fn &rest args)
    "Preserve `buffer-modified-p' during snippet preview.
consult-yasnippet previews by expanding snippets into the buffer,
which marks it as modified even on cancel."
    (let ((state-fn (apply orig-fn args))
          (modified-flag (buffer-modified-p)))
      (lambda (action template)
        (funcall state-fn action template)
        (set-buffer-modified-p modified-flag))))
  (advice-add 'consult-yasnippet--preview :around
              #'my-consult-yasnippet--preview))
