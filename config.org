...
Open REPL with RUN-REPL function if REPL is not yet opened. Evil-Leader z
binding will be added to toggle MODE and REPL buffer unless NOBIND is not nil."
...
If NOTICE is non-nil and Tmux is not running, print the message:
\"Tmux is not running!\""
...
(defun my-find-file-in-git-project (regex root)
  (when (file-exists-p (expand-file-name ".git" root))
    (let ((default-directory (file-truename root)))
      (seq-filter (lambda (f) (string-match-p regex f))
                  (sk-process-lines "git" "ls-files")))))
  ...
  (if-let* ((file (buffer-file-name))
            ...
            (regex
             (cond ((member ext '("h" "hpp"))
                    (rx (or bol "/") (literal base) "." (or "c" "cpp") eol))
                   ((member ext '("c" "cpp"))
                    (rx (or bol "/") (literal base) "." (or "h" "hpp") eol))
                   (t nil)))
...
(defun async-shell-command-callback (cmd callback &rest args)
  "Run CMD asynchronously, then call CALLBACK with ARGS."
...
When OMIT-CURLY-BRACES is non-nil, omit curly braces wrapping."
  (concat
...
Treat quoted strings and balanced parentheses, brackets, and braces as
single units."
  (interactive "r")
    ...
    (let* ((files (dired-get-marked-files))
      ...
      (if remote
        ...
        (let* ((deref-opt (if dired-copy-dereference "-L " ""))
          ...
          (async-shell-command-callback
           (add-sudo-to-command (> arg 1) cmd) refresh-dired target-files)))))
      ...
      (when (and files
        ...
        (if remote
          ...
          (let* ((source (mapconcat #'shell-quote-argument files " "))
            ...
            (async-shell-command-callback
             (add-sudo-to-command (> arg 1) cmd) refresh-dired))))))
...
Layer the `dired-directory' face underneath any existing face
properties so completion match highlighting remains visible."
    (if (string-suffix-p "/" file)
        (let ((copy (copy-sequence file)))
          (add-face-text-property 0 (length copy) 'dired-directory 'append copy)
          copy)
      file))
  (defun +vertico-kill-ring-no-truncate (orig str max)
    "Show `kill-ring' candidate STR multiline, capped to
`+vertico-kill-ring-max-lines' lines with a '[...]' marker.
Otherwise truncate via ORIG with MAX."
    (if (eq (vertico--metadata-get 'category) 'kill-ring)
        (let ((lines (split-string (string-trim-right str) "\n")))
          (if (> (length lines) +vertico-kill-ring-max-lines)
              (concat
               (string-join (seq-take lines +vertico-kill-ring-max-lines)
                            "\n")
               "\n[...]")
            (string-trim-right str)))
      (funcall orig str max)))
  (advice-add 'vertico--truncate-multiline :around
              #'+vertico-kill-ring-no-truncate)
  (cl-defmethod vertico--format-candidate :around
    (cand prefix suffix index start
          &context ((vertico--metadata-get 'category) (eql kill-ring)))
    "Draw a separator line after the candidate has been formatted."
    (let ((formatted (cl-call-next-method cand prefix suffix index start)))
      ;; No separator under the last candidate.
      (if (>= index (1- vertico--total))
          formatted
        (concat formatted (propertize (concat (make-string 40 ?―) "\n")
  ...
  (defun +vertico-kill-ring--height (cand)
    ...
    (let ((n (1+ (cl-count ?\n (string-trim-right cand)))))
      (+ (min +vertico-kill-ring-max-lines n)
         (if (> n +vertico-kill-ring-max-lines) 1 0)  ; "[...]" line
         1)))                                         ; separator line
  (defun +vertico-kill-ring--fit (scroll avail)
    "Number of candidates from SCROLL that fit AVAIL lines."
    (let ((used 0) (n 0))
      (cl-loop for c in (nthcdr scroll vertico--candidates)
               for h = (+vertico-kill-ring--height c)
               while (or (zerop n) (<= (+ used h) avail))
               do (setq used (+ used h)
                        n (1+ n)))
      (max 1 n)))
  (defun +vertico-kill-ring--center (index avail)
    "Scroll offset placing candidate INDEX about half a page down."
    (let ((budget (/ avail 2)) (used 0) (s index))
      (cl-loop for j from (1- index) downto 0
               for h = (+vertico-kill-ring--height (nth j vertico--candidates))
               while (<= (+ used h) budget)
               do (setq used (+ used h)
                        s j))
      s))
  (defun +vertico-kill-ring-page-scroll (orig)
    "Page-jump scrolling for multiline kill-ring candidates."
    (if (eq (vertico--metadata-get 'category) 'kill-ring)
        (let* ((win (and (overlayp vertico--candidates-ov)
                         (overlay-get vertico--candidates-ov 'window)))
               (side (window-live-p win))
               (avail
                (if side
                    (max 1 (- (/ (window-pixel-height win)
                                 (default-line-height))
                              (if mode-line-format 2 1)))
                  ;; Bottom minibuffer: cap to half the frame height, matching
                  ;; the `vertico-count' the side-style setup uses elsewhere.
                  (max 1 (/ (frame-height) 2))))
               (i (max 0 vertico--index))
               (s (or vertico--scroll 0))
               (page (+vertico-kill-ring--fit s avail)))
          (when (or (< i s) (>= i (+ s page)))   ; left page -> recenter
            (setq s (+vertico-kill-ring--center i avail)
                  page (+vertico-kill-ring--fit s avail)))
          ;; In the fixed-size side window the extra candidate fills the bottom
          ;; gap; the auto-growing minibuffer must stay within AVAIL, so no +1.
          (setq-local vertico-count (min (max 1 (- vertico--total s))
                                         (if side (1+ page) page)))
          (setq vertico--scroll s))
      (funcall orig)))
  (advice-add 'vertico--compute-scroll :around #'+vertico-kill-ring-page-scroll)
  (defun my-vertico-sort (input candidates)
    "Sort CANDIDATES with exact match first, then prefix matches of
INPUT's first word, then the rest.  Within each bucket, sort
alphabetically via `vertico-sort-alpha'."
    (if (or (string-empty-p input)
            (eq (aref input 0) ?^))
        (vertico-sort-alpha candidates)
      (let ((first-word (or (car (split-string input)) ""))
            res-exact res-prefix res-others)
        (dolist (cand candidates)
          (cond
           ((eq t (compare-strings input nil nil cand nil nil t))
            (push cand res-exact))
           ((string-prefix-p first-word cand t)
            (push cand res-prefix))
           (t
            (push cand res-others))))
        (nconc
         (vertico-sort-alpha res-exact)
         (vertico-sort-alpha res-prefix)
         (vertico-sort-alpha res-others)))))
  (defun vertico-default-sort (candidates)
    "Sort candidates with `my-vertico-sort'.
When the input is empty, return CANDIDATES in their original order so
that completion tables which supply a meaningful ordering (e.g.
`describe-face') are preserved."
