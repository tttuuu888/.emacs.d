...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defun find-file-in-tree (dir fname)
    "Find a file up to the HOME folder or root folder."
    (let ((file (expand-file-name fname dir))
          (parent (unless (or (equal "~/" dir) (equal "/" dir))
                    (file-name-directory (directory-file-name dir)))))
      (cond ((file-exists-p file) dir)
            (parent (find-file-in-tree parent fname))
            (t nil))))

  (defun my-makefile-process (&optional phony use-process)
    "Find a Makefile path and run 'make' with PHONY target.
  Use compile function unless USE-PROCESS is not nil."
    (let* ((dir (find-file-in-tree default-directory "Makefile"))
           (cmd (concat "export LANG=en_US && make -j8 -C " dir " " phony)))
      (cond ((equal dir nil) (message "Makefile is not found"))
            (use-process (call-process-shell-command cmd))
            (t (compile cmd)))))
  ...
  (defun sk-clean ()
    (interactive)
    (my-makefile-process "clean"))

  (defun sk-rebuild ()
    (interactive)
    (my-makefile-process "clean" t)
    (my-makefile-process))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package term
    :ensure nil
    :custom-face
    (term-color-blue ((t :foreground "blue4" :background "blue4")))
    :config
  ...
  (use-package ansi-color
    ...
    (ansi-color-names-vector
     ["black" "red3" "green3" "yellow3" "blue4" "magenta3" "cyan3" "gray90"])
    :config
  ...
  (use-package ivy
    ...
    (defun sk-ivy-buffer-transformer (str)
      (let* ((buf (get-buffer str))
             ...
             (max-path-len (max 0 (- (frame-width) 65 (max 0 (- buf-len 35)))))
             ...
             (path-margin (max 0 (- 20 (max 0 (- buf-len 35)))))
             (path (if path-mod
                       (concat path-prefix
                               "…"
                               (replace-regexp-in-string "^[^~/]*" "" path-mod))
                     path-opt))
             (form (concat "%-35s  %-" (int-to-string path-margin) "s  %s")))
