...
(use-package org
  ...
  (defun my-org-before-process-hook (_exporter)
    (cl-flet ((hex (c) (apply #'color-rgb-to-hex
                              (append (color-name-to-rgb c) '(2)))))
      (setq-local org-html-head
                  (concat org-html-head
                          (html-head-css
                            (format ".src { background-color: %s; color: %s; }\n"
                                    (hex (face-background 'default))
                                    (hex (face-foreground 'default))))))))
