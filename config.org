...
(use-package project
  ...
  (advice-add 'project--remember-dir :before-while
              (lambda (&rest _) sk-project-remember))
  ...
  (defun sk-project-try (dir)
    ...
    (when-let* ((root (sk-project-root dir)))
      (cons 'sk root)))
  (defun sk-project-prompt-project-dir ()
    "Prompt for one of the known project roots."
    (project--ensure-read-project-list)
    (let* ((dirs (project-known-project-roots))
           (table (project--file-completion-table dirs)))
      (unless dirs
        (user-error "There are no known projects"))
      (let (history-add-new-input)
        (completing-read "Select project: " table nil t nil nil (car dirs)))))
  ...
  (defun sk-project-save-buffers ()
    ...
    (let ((bufs (project-buffers (project-current t))))
      (save-some-buffers t (lambda () (memq (current-buffer) bufs)))))
