  ...
  (defun sk-package-upgrade-all ()
    ...
    (cl-letf (((symbol-function 'package--quickstart-maybe-refresh) #'ignore))
      ...
      (let* ((vc-pkgs (my-vc-packages))
        ...
        (sk-package-delete-obsolete)
        (when (and package-quickstart upgraded) (package-quickstart-refresh))
        (if (not upgraded)
...
(use-package corfu
  :defer-init (global-corfu-mode 1)
  :bind (:map corfu-map
          ("C-n"      . corfu-next)
          ("C-p"      . corfu-previous)
          ("<return>" . corfu-insert))
...
(use-package yasnippet-capf
  ...
  (defun sk-yasnippet-capf ()
    (let ((beg (save-excursion (skip-chars-backward "[:word:][:punct:]")
                               (point))))
      (when (and (< beg (point))
                 (not (looking-at "[[:word:][:punct:]]")))
...
(use-package consult
  ...
  (add-to-list 'consult-preview-allowed-hooks 'which-func-ff-hook)
...
(use-package project
  ...
  (cl-defmethod project-root ((project (head sk)))
    (cdr project))
  (advice-add 'project-try-vc :around
              (lambda (func &rest r)
                (let ((vc-handled-backends '(Git)))
                  (apply func r))))
  (defun sk-project--fd-files (dir)
    "List files under DIR with fd."
    (let* ((default-directory (file-name-as-directory dir))
           (names (with-temp-buffer
                    (process-file "fd" nil '(t nil) nil "-0" "-t" "f"
                                  "--color=never" "--strip-cwd-prefix")
                    (split-string (buffer-string) "\0" t))))
      (if project-files-relative-names
          names
        (mapcar (lambda (f) (concat default-directory f)) names))))
  (cl-defmethod project-files ((project (head sk)) &optional dirs)
    "Use git ls-files in a repository, else with fd."
    (let ((dirs (or dirs (list (cdr project)))))
      (or (when-let* ((vc (project-try-vc (cdr project))))
            (project-files vc dirs))
          (and (let ((default-directory (cdr project)))
                 (executable-find "fd" t))
               (mapcan #'sk-project--fd-files dirs))
          (cl-call-next-method))))
...
(use-package janet-mode
  :config
  (require 'inf-janet)
  (put 'with 'janet-indent-function 1)
