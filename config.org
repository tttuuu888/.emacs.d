...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package vertico-multiform
    ...
    (setq
     ...
     '((file (vertico-sort-function . vertico-sort-dir-first-alpha)
    ...
    (defun vertico-sort-dir-first-alpha (lst)
      "Sort directories before files, both sorted alphabetically."
      (let (dirs files)
        (dolist (x lst)
          (if (string-suffix-p "/" x)
              (push x dirs)
            (push x files)))
        (setq dirs (sort dirs #'string-lessp))
