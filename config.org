...
Some packages takes relatively long time to load and Emacs freeze while loading package. This setting make Emacs print notification that specific package is loading.
#+begin_src emacs-lisp
  (defvar sk-pacakge-loading-notice-list '(org yasnippet))

  (advice-add 'require :around
              (lambda (old pkg &rest r)
                (if (not (member pkg sk-pacakge-loading-notice-list))
                    (apply old pkg r)
                  (let* ((msg (capitalize (format "%s loading ..." pkg)))
                         (pos (point))
                         (endp (equal pos (point-max)))
                         (beg (if endp (- pos 1) pos))
                         (end (if endp  pos (+ pos 1)))
                         (ovr (make-overlay beg end)))
                    (message msg)
                    (overlay-put ovr 'display msg)
                    (overlay-put ovr 'face '(:bold t :inverse-video t))
                    (redisplay)
                    (apply old pkg r)
                    (delete-overlay ovr)
