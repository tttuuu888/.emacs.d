...
* Directory layout
A default derived from =user-emacs-directory= is frozen when its defining file
loads, and many of those files load before =config.el= is loaded. So the
directory layout is established in =early-init.el= as follows:

- =user-emacs-directory= is set to =~/.emacs.d/.local/= to isolate all
  package-generated local files.
...
(use-package org
  ...
  (defun sk-org-babel-tangle-async (&optional target-file)
    ...
    (interactive)
    (let* ((target (or target-file (buffer-file-name)))
           (sexp `(progn
                    (package-initialize)
                    (require 'org)
                    (org-babel-tangle-file ,target)))
           (elisp-cmd (prin1-to-string sexp))
           (shell-cmd (format "emacs -Q --batch --eval %s"
                              (shell-quote-argument elisp-cmd))))
      (if target
          (progn
            (start-process-shell-command "async-tangle-proc" nil shell-cmd)
...
(use-package helm
  ...
  (setq helm-allow-mouse nil
        ...
        helm-grep-input-idle-delay 0.01
  ...
  (defun sk-remove-known-project (project)
    ...
    (interactive
     (let* ((root (sk-project-root))
            ...
            (cands (if (and root (member root projectile-known-projects))
                       (cons root (remove root projectile-known-projects))
                     projectile-known-projects)))
       (list (projectile-completing-read
              "Remove from known projects: " cands))))
