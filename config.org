  ...
  (advice-add 'vertico-posframe--show
              ...
              (lambda (func &rest args)
                (setq vertico-posframe-truncate-lines
                      (< (point)
                         (* 0.8 (window-width (active-minibuffer-window))))))))
...
(use-package consult
  ...
  (defun sk-compile-scripts ()
    (if-let ((sbin (file-name-as-directory
                    (find-file-in-tree default-directory ".sbin"))))
        (let (files-alist)
          (dolist (f (directory-files sbin t "\\.sh$"))
            (push (cons (file-name-nondirectory f) f) files-alist))
          (cons sbin (nreverse files-alist)))
