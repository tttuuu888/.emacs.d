...
#+BEGIN_SRC emacs-lisp
  (defun my-theme-setting (&optional light-theme)
    (let ((fg-color (if light-theme "Black" "Gray80"))
          (bg-color (if light-theme "White" "Gray23")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun find-file-in-tree (dir filename &optional return-only-dir)
    "Find file FILENAME up to root folder."
    (let ((file (expand-file-name filename dir))
          (parent (file-name-directory (directory-file-name dir))))
      (cond ((equal dir parent) nil)
            ((file-exists-p file)
             (cond (return-only-dir dir)
                   ((file-directory-p file) (file-name-as-directory file))
                   (t file)))
            (parent (find-file-in-tree parent filename return-only-dir))
            (t nil))))
  ...
  (defun sk-clang-complete-make ()
    ...
    (let ((file "./.clang_complete")
          (includes (shell-command-to-string
                     "find -L -type f -name '*.h' -printf '-I%h\n' | sort -u")))
  ...
  (defun sk-project-root (&optional dir strict)
    (let ((dir (or dir default-directory))
          (found nil)
          (checks '(".git" ".projectile")))
      (cl-loop for c in checks
               until found
               do (when-let ((root (find-file-in-tree dir c t)))
                    (setq found root)))
      (if strict
          (and found (file-truename found))
        (file-truename (or found dir)))))

  (defun sk-project-try-vc (dir)
    (let ((root (sk-project-root dir t)))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package sh-script
    ...
    (add-to-list 'auto-mode-alist '("\\.te\\'" . sh-mode)))
  ...
  (use-package project
    :ensure nil
    :config
    (setq project-find-functions '(sk-project-try-vc)))
#+END_SRC
    ...
    (cl-defmethod eglot-client-capabilities ((server eglot-pyright))
      ...
      `(:workspace
        (:didChangeWatchedFiles
         (:dynamicRegistration
          ,(if (sk-project-try-vc default-directory)
               t
             :json-false)))))
