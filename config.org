...
** User information
If =USER_FULL_NAME= and =USER_MAIL_ADDRESS= environment variables are set,
update =user-full-name= and =user-mail-address= variables.
...
(defun use-package-handler/:defer-init (name _keyword args rest state)
  ...
  (let ((fn (intern (format "defer-init/%s" (use-package-as-symbol name)))))
    ...
    (let ((avail (+vertico-kill-ring--avail)) ; available lines
      ...
      (cl-loop for cand in vertico--candidates for i from 0
               ...
               do (when (and (> count 0) (> (+ used h) avail)) ; page is full
                  ...
                  (setq used (+ used h) count (1+ count)))
