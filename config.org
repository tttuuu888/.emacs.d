...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package ox-latex
    ...
    (add-to-list 'org-latex-classes
                 '("awesomecv" "\\documentclass{awesome-cv} [NO-DEFAULT-PACKAGES]"
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package eglot
    ...
    (defun my-eglot-start (&rest _)
      (let* ((file (buffer-file-name))
             (proj (cdr (eglot--current-project)))
             (my-proj-list (gethash proj my-eglot-projects)))
        (puthash proj (cons file my-proj-list) my-eglot-projects)))
  
    (defun my-eglot-kill-file (&rest _)
      (let* ((file (buffer-file-name))
             (my-projs (hash-table-keys my-eglot-projects)))
        (dolist (proj my-projs)
          (let ((my-proj-list (gethash proj my-eglot-projects)))
            (when (member file my-proj-list)
              (puthash proj (delete file my-proj-list) my-eglot-projects)
              (unless (delete file my-proj-list)
                (eglot-shutdown (eglot-current-server))
                (remhash proj my-eglot-projects)))))))
  
    (defun my-eglot-open-file (&rest _)
      (let* ((file (buffer-file-name))
             (my-projs (hash-table-keys my-eglot-projects)))
        (dolist (proj my-projs)
          (when (string-match-p proj file)
            (puthash proj
                     (cons file (gethash proj my-eglot-projects))
                     my-eglot-projects)))))
  
    (add-hook 'eglot-server-initialized-hook 'my-eglot-start)
    (add-hook 'kill-buffer-hook 'my-eglot-kill-file)
    (add-hook 'find-file-hook 'my-eglot-open-file)
