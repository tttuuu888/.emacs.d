...
Since a default derived from =user-emacs-directory= is frozen when its defining
file loads, and many of those files load before =config.el= does, the directory
layout is established in =early-init.el= as follows:

- =user-emacs-directory= is set to =~/.emacs.d/.local/= to isolate all
  package-generated files.
- =emacs-config-dir= is used instead of =user-emacs-directory= to refer to the
  configuration root.
- =emacs-etc-dir= indicates the folder containing extra configurations.
- =package-user-dir= is pinned to =~/.emacs.d/elpa/=, outside =.local/=.

=init.el= will generate =config.el= from =config.org= and then will load
=config.el=.

Everything else is configured in =config.org=.
...
** Package initialize
=package-enable-at-startup= is nil in =early-init.el=, so activation is
explicit. =package-activate-all= skips =package-read-all-archive-contents=;
package commands initialize it on demand. =package-quickstart-file= is set here
since its default is fixed even earlier.

#+BEGIN_SRC emacs-lisp
(setq package-archives '(("gnu"   . "https://elpa.gnu.org/packages/")
                         ("melpa" . "https://melpa.org/packages/"))
      package-archive-priorities '(("gnu"   . 5) ("melpa" . 10))
      package-quickstart t
      package-quickstart-file (locate-user-emacs-file "package-quickstart.el"))
...
(use-package use-package
  ...
  (defun use-package-handler/:defer-init (name _keyword args rest state)
    "Generate code to defer ARGS until the first command after init."
    (let ((fn (intern (format "defer-init/%s" name))))
...
(use-package emacs
  ...
  ;; custom.el will be saved but will not be used.
  (custom-file (expand-file-name "custom.el" user-emacs-directory))
  (default-input-method "korean-hangul3")
  (fill-column 80)
  (font-use-system-font t)
  (gc-cons-threshold (* 1024 1024 16))
...
(use-package emacs
  ...
  (defvar sk-project-root-files
    '(".project" ".git"
      "CMakeLists.txt" "Makefile" "makefile.mk" "GTAGS"
      "Android.bp" "Cargo.toml"))

  (defvar-local sk-vc-mode nil)
  ...
  (defmacro sk-switch-buffer-repl (name mode repl run-repl &optional nobind)
    "Define NAME to toggle between MODE and REPL buffers.
When called, NAME switches from MODE to REPL, running RUN-REPL if needed,
or back to MODE.  Unless NOBIND is non-nil, binds <leader> z in both modes."
    ...
    `(defun ,name (&rest _)
       ...
       (if (region-active-p)
         ...
         (let* ((curr (min (1+ (point)) (point-max)))
           ...
           (setq this-command ',eval-region)
           (,eval-region back curr)))
       (setq deactivate-mark t)))

  (defmacro sk-with-sudo (sudo &rest body)
    "Evaluate BODY as root via TRAMP when SUDO is non-nil, or normally if nil.
BODY should not switch buffers or rebind `default-directory'."
    (declare (indent 1))
    `(let ((default-directory (if ,sudo
  ...
  (defun find-file-in-tree (dir files &optional return-only-dir)
    "Find the first matching file from FILES (a list) up to home or root.
If RETURN-ONLY-DIR is non-nil, return its directory instead of the full path."
    (let ((cur-dir (file-name-as-directory (expand-file-name dir)))
          (files (ensure-list files))
          (home (expand-file-name "~/")))
      (cl-loop
       for all-files = (and (file-readable-p cur-dir)
                            (directory-files cur-dir))
       for found = (cl-intersection files all-files :test #'string=)
       for parent = (file-name-directory (directory-file-name cur-dir))
       if found return (if return-only-dir
                           cur-dir
                         (expand-file-name (car found) cur-dir))
       else if (or (equal cur-dir home)
                   (equal cur-dir parent))
       return nil
       else do (setq cur-dir parent))))

  (defun sk-process-lines (program &rest args)
    "Like `process-lines-ignore-status', but honor remote `default-directory'."
    (with-temp-buffer
    ...
    (let* ((file "./.clang_complete")
           (dirs (delete-dups
                  (sk-process-lines
                   ...
                   "-not" "-path" "*/.*" "-printf" "-I%h\\n")))
           (includes (mapconcat (lambda (d) (concat d "\n"))
                                (sort dirs #'string<) "")))
  ...
  (defun hide-ctrl-M ()
    "Hides the '^M' in files containing mixed UNIX and DOS line endings."
    (interactive)
  ...
  (defun sk-C-c-map-to-leader-c-map (mode mode-map &optional exceptions)
    "Bind MODE-MAP's C-c keys to <leader> c for MODE, except EXCEPTIONS list."
    (let* ((ret nil)
             ...
             (if (symbolp elt)
               ...
               (dolist (k ks)
                 ...
                 (let ((nelt (alist-get k elt))
                   ...
                   ;; Ignore multi-char keys. e.g. "SPC" would be read as S, P, C.
                   (when (and nelt (= 1 (length (single-key-description npre))))
                     (ctrl-map-search nelt (append pre (list npre)))))))))
        (ctrl-map-search target (list ?\c))
        (setq ret (nreverse ret))
        (dolist (e ret)
          (let ((key (mapconcat 'single-key-description (car e) "")))
            (unless (cl-some (lambda (ex) (string-prefix-p ex key)) exceptions)
              (evil-leader/set-key-for-mode mode key (cdr e))))))))

  (defun tmux-running-p (&optional notice)
    "Return non-nil if a tmux session exists.
If NOTICE is non-nil, display a message when no session is running."
    (let ((running (zerop (call-process "tmux" nil nil nil "has-session"))))
      (when (and (not running) notice)
        (message "No tmux session is running!"))
      running))
  ...
  (defun sk-create-project (dir)
    "Create a .project file in DIR, if not exists."
    (write-region "" nil (expand-file-name ".project" dir) t))

  (defun get-clear-buffer (buffer)
    "Return BUFFER after clearing its contents, creating it if needed."
    (with-current-buffer (get-buffer-create buffer)
      (erase-buffer)
      (current-buffer)))

  (defun my-git-output (&rest args)
    "Run git with ARGS and return the output, or nil on failure."
    (with-temp-buffer
      (when (zerop (apply #'process-file "git" nil t nil args))
        (string-trim-right (buffer-string)))))

  (defun git-tracked-file-p (&optional file-name)
    (when-let* ((file (or file-name buffer-file-name))
  ...
  (defun git-branch-of-file (file)
    (when (git-tracked-file-p file)
      (let* ((default-directory (file-name-directory (expand-file-name file)))
             (branch (my-git-output "branch" "--show-current")))
        (if (equal branch "")
            (my-git-output "rev-parse" "--short" "HEAD")
          branch))))
    ...
    (when (file-exists-p (expand-file-name ".git" root))
      (let ((default-directory (file-truename root)))
        (seq-filter
         ...
         (sk-process-lines "git" "ls-files" "-co" "--exclude-standard")))))

  (defun sk-git-diff-relative ()
    "Run git diff with --relative option."
    (interactive)
    (if (zerop
         (process-file "git" nil (get-clear-buffer "*git-diff*") nil
  ...
  (defun my-find-file-in-dir (regex &optional dir)
    (directory-files (or dir default-directory) nil regex))

  (defun sk-find-other-file ()
    "Switch between C/C++ source and header files in the project."
    (interactive)
    (let ((inhibit-message t))
      (if-let*
          ((file (buffer-file-name))
           ...
           (regex
            (cond ((member ext '("h" "hh" "hpp"))
                   (rx (or bol "/") (literal base) "." (or "c" "cc" "cpp") eol))
                  ((member ext '("c" "cc" "cpp"))
                   (rx (or bol "/") (literal base) "." (or "h" "hh" "hpp") eol))
                  (t nil)))
           (others (or (my-find-file-in-dir regex)
                       (and (setq root (my-get-git-root-dir))
                            (my-find-file-in-git-project regex root)))))
          (find-file
           (expand-file-name (if (length= others 1)
                                 (car others)
                               (consult--read others :prompt "Switch to: "))
                             root))
        (user-error "No other file found"))))

  (defun async-shell-command-callback (cmd sudo callback &rest args)
    "Run CMD asynchronously as root if SUDO, then call CALLBACK with ARGS.
CALLBACK runs only when CMD exits successfully.  Return the process."
    (let* ((buffer (generate-new-buffer "*Async Shell Command*"))
           (proc (sk-with-sudo sudo
                   (start-file-process "Async" buffer shell-file-name
       ...
       (lambda (process _signal)
         (when (memq (process-status process) '(exit signal))
           (if (zerop (process-exit-status process))
             ...
             (message "Command failed (%d): %s"
                      (process-exit-status process) cmd)))))
      proc))
  ...
  (defun sk-alist-to-js-obj (alist &optional omit-curly-braces)
    "Convert ALIST to JavaScript object string.
If OMIT-CURLY-BRACES is non-nil, omit the surrounding curly braces."
    (concat
  ...
  (defun sk-google-search (&optional beg end)
    (interactive "R")
    (let ((target (if beg
  ...
  (defun align-columns (beg end)
    "Align columns in the region between BEG and END.
Treat quoted strings and balanced delimiters as single units."
    (interactive "r")
  ...
  (defun ensure-dir (dir)
    "Return DIR, creating it and any parent directories if needed."
  ...
  (defun sk-add-comment-start ()
    "Add `comment-start' setting to the buffer header."
    (interactive)
    (let* ((cs (or comment-start
                   (read-string "No comment syntax is defined.  Use: ")))
           (comment-format (format " -*- comment-start: \"%s\"; -*-" cs cs)))
      (setq-local comment-start cs)
      (save-excursion
        (goto-char (point-min))
        (when (looking-at "#!") (forward-line 1))
        (if (re-search-forward "-*- comment-start:" (line-end-position) t)
            (message "Comment setting already present on first line.")
          (if (not (looking-at cs))
              (insert cs comment-format "\n")
            (end-of-line)
            (insert comment-format))
          (message "Added comment-start: %s" cs)))))

  (defun html-head-css (&rest body)
    (concat "<style type=\"text/css\">\n"
            (string-join body)
            "</style>\n"))

  (defun html-head-js (&rest body)
...
(use-package emacs
  ...
  (when (display-graphic-p)
    ...
    (set-fontset-font t 'unicode "Unifont Upper" nil 'prepend)
  ...
  (defun my-theme-setting (&optional light-theme)
    (let ((base-fg-color   (if light-theme "Black" "Gray80"))
          (base-bg-color   (if light-theme "Gray95" "Gray23"))
     ...
     '("%e"
       (:eval
        (let* ((width (- (window-width)
               ...
               (left-str (concat evil-info buffer-info buffer-name mode-and-vc))
               (left-width (- width right-len))
               (left-info (if (<= (string-width left-str) left-width)
                              left-str
                            (truncate-string-to-width left-str left-width)))
...
(use-package async
  :init
  (setq async-bytecomp-not-allowed-packages
        '(cond-let llama magit magit-section transient with-editor))
  (defun async--package-compile (orig-fun pkg-desc &rest args)
    (let ((cur-package (package-desc-name pkg-desc))
          (pkg-dir (package-desc-dir pkg-desc)))
      (if (and (or (member async-bytecomp-allowed-packages '(t all (all)))
                   (memq cur-package (async-bytecomp--get-package-deps
                                      async-bytecomp-allowed-packages)))
               (not (memq cur-package async-bytecomp-not-allowed-packages)))
          (progn
  ...
  (defun my-async--emacs-program-args-advice (orig-fun &rest args)
    "Let the child Emacs share the parent's eln-cache."
    (append (list "--eval" (format "(startup-redirect-eln-cache %S)"
...
(use-package evil-leader
  ...
  (add-hook 'after-change-major-mode-hook 'sk-evil-leader-mode t)
  ...
  (defun sk-evil-leader-restore (&rest _)
    (sk-evil-leader-mode)
    (evil-change-to-initial-state))
  (dolist (f '(grep-edit-save-changes
               wdired-change-to-dired-mode
               xref-edit-save-changes))
    (advice-add f :after #'sk-evil-leader-restore)))
...
(use-package evil
  ...
  (unless (display-graphic-p)
    (defun my-evil-term-set-cursor (&rest _args)
      "Sync terminal cursor shape with the `cursor-type' updated by Evil."
      (let ((type (car (ensure-list cursor-type))))
        (cond
         ((eq type 'bar)  (send-string-to-terminal "\e[6 q"))
         ((eq type 'hbar) (send-string-to-terminal "\e[4 q"))
         (t               (send-string-to-terminal "\e[2 q")))))
    (advice-add 'evil-set-cursor :after #'my-evil-term-set-cursor)))
...
(use-package files
  :ensure nil
  :defer-init
  (dolist (dir '("auto-save-list" "eln-cache" "snippets" "transient"))
    (let ((path (expand-file-name dir emacs-config-dir)))
...
(use-package calendar
  ...
  :bind (:map calendar-mode-map
          ...
          ("C-f"     . calendar-scroll-calendar-left)
          ("C-b"     . calendar-scroll-calendar-right)
...
(use-package dired
  ...
  (evil-leader/set-key-for-mode 'wdired-mode
    ...
    (let ((files (if files files '(nil)))
      ...
      (expand-file-name
       (minibuffer-with-setup-hook
           (lambda () (setq-local minibuffer-default-add-function nil))
         (dired-mark-read-file-name
          prompt default-directory 'copy nil files defaults)))))
  ...
  (defun my-dired-rsync ()
    ...
    (let* ((files (dired-get-marked-files nil current-prefix-arg))
           ...
           (port (when-let* ((remote (or dst-remote-p src-remote-p)))
                   (tramp-file-name-port (tramp-dissect-file-name remote))))
           ;; "/ssh:u@host#2222:/d" -> "u@host:/d"
           (spec (lambda (f)
                   (let ((v (tramp-dissect-file-name f)))
                     (concat (when-let* ((u (tramp-file-name-user v)))
                               (concat u "@"))
                             (tramp-file-name-host v) ":"
                             (file-local-name f)))))
           (target (if dst-remote-p (funcall spec target) target))
           (cmd (concat "rsync -ahs"
                        (when (or src-remote-p dst-remote-p) "z")
                        (when port (format " -e 'ssh -p %s'" port))
                        " --info=progress2 "))
           (arg (mapconcat
  ...
  (defun my-dired-compress (&optional arg)
    ...
    (interactive "p")
    (let* ((defaults (delete (dired-current-directory)
                             (dired-dwim-target-defaults nil nil)))
           (files (mapcar #'file-name-nondirectory (dired-get-marked-files)))
           (source (mapcar #'shell-quote-argument files))
           (initial-name (if (equal (length files) 1)
                             (if (dired-nondirectory-p (car files))
                                 (file-name-base (car files))
                               (file-name-nondirectory (car files)))
                           (file-name-nondirectory
                            (directory-file-name default-directory))))
           (initial-name (if (or (string-empty-p initial-name)
                                 (string= initial-name "~"))
                             "default"
                           initial-name))
           (name (consult--read nil
                                :prompt "Compress file name: "
                                :initial (concat initial-name ".zip")
                                :default defaults))
           (target (shell-quote-argument name))
           (ext (file-name-extension name))
           (pw (when (> arg 1)
                 (if (member ext '("zip" "7z"))
                     (password-read "Password: ")
                   (user-error "Password is supported only for zip and 7z"))))
           (cmd (if (string-match-p (rx ".tar." (+ (not ".")) eos) name)
                    "tar acvf "
                  (concat "7z a "
                          (when (equal ext "7z") "-md=1m ")
                          (when pw (if (equal ext "7z")
                                       "-mhe=on -p "
                                     "-mem=AES256 -p "))))))
      (when ext
        (let ((proc (async-shell-command-callback
          ...
          (when pw
            (process-send-string proc pw)
            (process-send-string proc "\n")
            (process-send-eof proc)
            (clear-string pw))))))

  (defun my-dired-do-shell-command (&optional arg)
    "Decompress if archive, otherwise run `dired-do-shell-command'."
    (interactive "p")
    (let* ((files (dired-get-marked-files))
           (exts (mapcar #'file-name-extension files))
           (archive-exts
            '("zip" "7z" "rar" "tar" "gz" "bz2" "xz" "zst" "tgz" "001")))
      (if (cl-every (lambda (ext) (member ext archive-exts)) exts)
          (my-dired-decompress arg)
        (call-interactively #'dired-do-shell-command))))

  (defun my-dired-decompress (&optional arg)
    "Decompress files asynchronously."
    (interactive "p")
    (let* ((files (dired-get-marked-files))
           (pws nil)
           (cmds
            (mapcar
             (lambda (f)
               (let* ((name (file-name-nondirectory f))
                      ...
                      (ext (file-name-extension name))
                      (pw (when (and (> arg 1) (member ext '("zip" "7z" "001")))
                            (password-read "Password: "))))
                 (when pw (push pw pws))
                 (if (string-match-p (rx ".tar." (+ (not ".")) eos) name)
                     (concat "tar xvf " qname)
                   (concat "7z x -y " (unless pw "-p ") qname))))
             files))
           (cmd (string-join cmds " && "))
           (proc (async-shell-command-callback
                  cmd nil
                  (lambda ()
                    (revert-buffer)
                    (message "Decompress files done.")))))
      (when pws
        (dolist (pw (nreverse pws))
          (process-send-string proc pw)
          (process-send-string proc "\n")
          (clear-string pw))
        (process-send-eof proc))))
  ...
  (defun my-dired-do-copy (&optional arg)
    "Copy files asynchronously for local, and synchronously for remote.
- Default: normal copy.
- M-0 : toggle dereference.
- M-2 : sudo copy.
- M-10: toggle dereference + sudo copy."
    (interactive "p")
    (let* ((files (dired-get-marked-files))
           (target (my-dired-get-target "Copy to: " files))
           (remote (or (file-remote-p default-directory)
                       (file-remote-p target)))
           (is-dir (directory-name-p target))
           (target-files (if (not is-dir)
                             (list target)
                           (mapcar
                            (lambda (f) (expand-file-name
                                         (file-name-nondirectory f) target))
                            files)))
           (dired-copy-dereference (if (zerop (% arg 10))
                                       (not dired-copy-dereference)
                                     dired-copy-dereference))
           (refresh-dired (lambda (&rest _)
                            (dolist (dest target-files)
                              (dired-remove-file dest)
                              (dired-add-file dest ?C))
                            (message "Copy files done."))))
      (when (and remote (> arg 1))
        (user-error "Cannot use sudo on remote files"))
      (if remote
          (progn
            (dolist (f files)
              (dired-copy-file f (if is-dir target (car target-files)) t))
            (funcall refresh-dired))
        (let* ((deref-opt (if dired-copy-dereference "-L " ""))
               (source (mapconcat #'shell-quote-argument files " "))
               (dest (shell-quote-argument target))
               (cmd (concat "cp -rf " deref-opt source " " dest)))
          (async-shell-command-callback
           cmd (> arg 1) refresh-dired target-files)))))

  (defun my-dired-do-delete (&optional arg)
    "Delete files asynchronously for local, and synchronously for remote."
    (interactive "p")
    (let* ((remote (file-remote-p default-directory))
           (files (dired-get-marked-files))
           (refresh-dired (lambda (&rest _)
                            (dolist (f files)
                              (dired-delete-entry f))
            ...
            (progn
              (dolist (f files)
                (if (file-directory-p f)
                  ...
                  (delete-file f)))
              (funcall refresh-dired))
          (let* ((source (mapconcat #'shell-quote-argument files " "))
                 (cmd (concat "rm -rf " source)))
            (async-shell-command-callback cmd (> arg 1) refresh-dired))))))
  ...
  (defun sk-sbin-dired ()
    (interactive)
    (if-let* ((target (locate-dominating-file default-directory ".sbin"))
              (sbin (expand-file-name ".sbin" target)))
        (dired sbin)
...
(use-package org
  ...
  (setq
   ...
   (concat
    (html-head-css (file-to-string (expand-file-name "org.css" emacs-etc-dir)))
    (html-head-js (file-to-string (expand-file-name "org.js" emacs-etc-dir))))
   ...
   org-latex-toc-include-unnumbered t
   org-log-done 'time
  ...
  (defun sk-org-capture-image ()
    ...
    (let* ((img (file-relative-name
                 (read-file-name "Image file name: ")
                 default-directory))
  ...
  (defun sk-org-babel-tangle-async (&optional target-file)
    "Do org-babel-tangle asynchronously."
    (interactive)
    (if-let* ((target (or target-file (buffer-file-name))))
        (progn
          (async-start
           `(lambda ()
           ...
           #'ignore)
          (message "Tangle code blocks from %s asynchronously."
                   (file-name-nondirectory target)))
      (user-error "No file to tangle.")))

  (add-hook 'org-mode-hook
            (lambda ()
              (let* ((config (expand-file-name "config.org" emacs-config-dir))
                     (target (cond
                              ((buffer-file-name) (buffer-file-name))
                              ((org-src-edit-buffer-p)
                               (buffer-file-name (org-src-source-buffer)))
                              (t nil))))
                (when (and target (string-equal target config))
                  (add-hook 'after-save-hook #'sk-org-babel-tangle-async
                            nil t))))))
...
(use-package ibuffer
  ...
  (define-ibuffer-sorter mode-name-then-filename
    "Sort buffers by mode name, then by file or process name."
...
(use-package display-line-numbers
  ...
  :hook
  ((find-file prog-mode text-mode) . sk-line-numbers)
  :config
  (setq-default display-line-numbers-width 3
                display-line-numbers-type 'visual
                display-line-numbers-current-absolute nil)
  (defun sk-line-numbers ()
    (unless display-line-numbers-mode (display-line-numbers-mode 1))))

(use-package tramp
  :ensure nil
  :init
  (defun sk-tramp-load-on-slash ()
    "Load TRAMP when / is typed in a file name minibuffer."
    (when (and (eq minibuffer-completion-table 'read-file-name-internal)
               (eq (char-before) ?/))
      (require 'tramp)))
  (add-hook 'post-self-insert-hook #'sk-tramp-load-on-slash)
  :config
  (remove-hook 'post-self-insert-hook #'sk-tramp-load-on-slash)
...
(use-package autorevert
  ...
  (defun my-remote-buffer-p (buffer)
    (file-remote-p (buffer-local-value 'default-directory buffer)))
  (setq-default global-auto-revert-ignore-buffer #'my-remote-buffer-p))
...
(use-package package
  ...
  :config
  (require 'package-vc)
  (defun sk-package-resync ()
    "Rebuild `package-alist' from the contents of `package-user-dir'."
    (setq package-alist nil)
    (package-load-all-descriptors))

  (defun sk-package-delete-obsolete ()
    "Delete every installed package version that is not the newest one."
    (interactive)
    (sk-package-resync)
    (dolist (entry (copy-sequence package-alist))
      (dolist (old (cddr entry))
        (package-delete old 'force 'dont-unselect)))
    (sk-package-resync))
  ...
  (defun sk-package-upgrade-all ()
    ...
    (cl-letf (((symbol-function 'package--quickstart-maybe-refresh) #'ignore))
      (sk-package-delete-obsolete)
      (let* ((vc-pkgs (my-vc-packages))
             (upgraded-vc (my-upgrade-vc-packages))
             (upgradeable-std (seq-difference (package--upgradeable-packages)
                                              vc-pkgs))
             (upgraded-std
              (seq-keep
               (lambda (pkg)
                 (when-let* ((desc (cadr (assq pkg package-archive-contents))))
                   ...
                   (and (assq pkg package-alist) pkg)))
               upgradeable-std))
             (upgraded (append upgraded-std upgraded-vc)))
        (sk-package-delete-obsolete)
        (when package-quickstart (package-quickstart-refresh))
        (if (not upgraded)
            (message "No packages to upgrade.")
          (message "%s packages upgraded. %s" (length upgraded) upgraded))))))
...
(use-package package-vc
  ...
  (defun package-vc-upgrade (pkg-desc)
    ...
    (let* ((vc-handled-backends '(Git))
           ...
           (pre-hash (my-git-output "rev-parse" "HEAD"))
           (_ (call-process "git" nil nil nil "pull"))
           (post-hash (my-git-output "rev-parse" "HEAD")))
      (when (not (equal pre-hash post-hash))
        (with-demoted-errors "Failed to activate: %S"
          (package-vc--unpack-1 pkg-desc pkg-dir)))))

  (defun my-vc-packages ()
    "Return a list of VC packages under `package-user-dir'."
    (let ((default-directory (expand-file-name package-user-dir)))
      (seq-filter
       (lambda (pkg)
         (file-directory-p
          (expand-file-name (concat (symbol-name pkg) "/.git"))))
       (mapcar #'intern (directory-files "." nil (rx bos (not ".")))))))

  (defun my-upgradable-vc-packages ()
    "Update all VC packages and return the list of updated ones."
    (cl-labels
        ((pull-vc-pkg (pkg)
           (async-start
            (lambda ()
              (let* ((default-directory
                      (expand-file-name (symbol-name pkg) package-user-dir))
                     (pre (car (process-lines-ignore-status
                                "git" "rev-parse" "HEAD")))
                     (_ (call-process "git" nil nil nil "pull" "-q"))
                     (post (car (process-lines-ignore-status
                                 "git" "rev-parse" "HEAD"))))
                (unless (string= pre post) pkg))))))
      (let ((futures (mapcar #'pull-vc-pkg (my-vc-packages))))
        (delq nil (mapcar #'async-get futures)))))

  (defun my-upgrade-vc-packages ()
    "Attempt to upgrade VC packages and return the list of upgraded ones."
    (let ((vc-pkgs (my-upgradable-vc-packages)))
      (dolist (pkg vc-pkgs)
        (when-let* ((desc (package-get-descriptor pkg)))
          (package-vc--unpack-1 desc (package-desc-dir desc))))
      vc-pkgs)))
...
(use-package vc
  ...
  :init
  (defvar sk-vc-tick 0
    "Bump whenever the branch of some repository may have changed.")
  (defvar-local sk-vc-buffer-tick -1
    "Value of `sk-vc-tick' when this buffer last read its branch.")
  (defun sk-vc-refresh-state ()
    (setq sk-vc-mode (git-branch-of-file (buffer-file-name))
          sk-vc-buffer-tick sk-vc-tick))
  (defun sk-vc-refresh-stale (&optional frame)
    "Update `sk-vc-mode' in the file buffers of FRAME that are out of date."
    (dolist (window (window-list frame 'no-minibuffer))
      (with-current-buffer (window-buffer window)
        (when (and buffer-file-name (/= sk-vc-tick sk-vc-buffer-tick))
          (sk-vc-refresh-state)))))
  (defun sk-vc-invalidate ()
    "Mark every buffer as out of date, and update the visible ones."
    (setq sk-vc-tick (1+ sk-vc-tick))
    (sk-vc-refresh-stale))
  (defun sk-vc-mark-stale ()
    "Mark the branch info of this buffer as out of date."
    (setq sk-vc-buffer-tick (1- sk-vc-tick))
    (when (get-buffer-window nil 'visible)  ; Update immediately if visible
      (sk-vc-refresh-state)))
  (add-hook 'after-set-visited-file-name-hook #'sk-vc-refresh-state)
  (add-hook 'after-revert-hook #'sk-vc-mark-stale)
  (add-hook 'find-file-hook #'sk-vc-refresh-state)
  (add-hook 'window-buffer-change-functions #'sk-vc-refresh-stale)
  :config
...
(use-package dabbrev
  :ensure nil
  :config
  (advice-add 'dabbrev-capf :before
              (lambda (&rest _)
...
(use-package corfu
  :bind (:map corfu-map
          ...
          ("<return>" . corfu-insert))
  :init
  (defun sk-corfu-enable ()
    (require 'corfu))
  (add-hook 'evil-insert-state-entry-hook #'sk-corfu-enable)
  :config
  (remove-hook 'evil-insert-state-entry-hook #'sk-corfu-enable)
  (require 'savehist)
  (setq corfu-auto t
        corfu-min-width 20
...
(use-package corfu-terminal
  ...
  :vc (:url "https://codeberg.org/akib/emacs-corfu-terminal.git"
            :rev :newest)
  :if (not (display-graphic-p))
  :defer-init (corfu-terminal-mode 1))
...
(use-package wgrep
  :config
  (dolist (mode '(grep-mode helm-ag-mode helm-git-grep-mode helm-grep-mode))
...
(use-package ox-reveal
  ...
  (setq my-reveal-init-options '((progress . false)
                                 (slideNumber . "c/t")
                                 (transition . "none"))
        org-reveal-plugins '(highlight markdown zoom notes)
...
(use-package magit
  ...
  (add-hook 'git-commit-setup-hook (lambda () (setq-local fill-column 72)))
  (add-hook 'magit-post-refresh-hook #'sk-vc-invalidate)
  ...
  (defun sk-save-git-diff (diff-file)
    ...
    (if (not (magit-staged-files))
        (message "No staged files")
      (with-temp-buffer
        (process-file "git" nil t nil "--no-pager" "diff" "--cached")
        (write-region nil nil diff-file))))
  ...
  (defun my-magit-insert-ref-line (name face msg)
    "Insert a \"  NAME  MSG\" line padded to the primary column."
  ...
  (defun my-magit-insert-tags-fast ()
    "Insert remote tags as plain text without section objects."
    (when-let* ((tags (magit-git-lines "tag" "--list" "-n"
                                       magit-buffer-refs-args)))
      (insert (propertize "Tags" 'font-lock-face 'magit-section-heading)
              (format " (%d)\n" (length tags)))
      (dolist (tag tags)
        (string-match "^\\([^ \t]+\\)[ \t]+\\([^ \t\n].*\\)?" tag)
        (let ((name (match-string 1 tag))
              (msg (match-string 2 tag)))
          (my-magit-insert-ref-line name 'magit-tag msg)))
      (insert "\n")))

  (defun my-magit-insert-remote-branches-fast ()
    "Insert remote branches as plain text without section objects."
    (dolist (remote (magit-list-remotes))
      (let ((lines (seq-remove
                    (lambda (line)
                      (string-equal (car (split-string line "\0")) remote))
                    (magit-git-lines "for-each-ref"
                                     "--format=%(refname:short)%00%(subject)"
                                     (concat "refs/remotes/" remote))))
            (pull (magit-get "remote" remote "url"))
        ...
        (insert
         ...
         (format " (%d)\n" (length lines)))
        (dolist (line lines)
          (pcase-let ((`(,branch ,msg) (split-string line "\0")))
            (let ((name (string-remove-prefix (concat remote "/") branch)))
              (my-magit-insert-ref-line name 'magit-branch-remote msg)))))
      (insert "\n")))

  (defun magit-light-mode ()
    "Speed up Magit by disabling heavy features."
    (setq-local
     ...
     magit-refs-sections-hook '(magit-insert-error-header
                                ...
                                my-magit-insert-remote-branches-fast
                                my-magit-insert-tags-fast)
     magit-revision-insert-related-refs nil
     magit-status-sections-hook
     (cl-remove-if
      (lambda (x)
        (memq x '(magit-insert-unpushed-to-pushremote
...
(use-package expand-region
  ...
  (defun my-er-plain-excursion ()
    (kill-local-variable 'er/save-mode-excursion))
  (add-hook 'org-mode-hook #'my-er-plain-excursion 90))
...
(use-package htmlize
  ...
  :after org
  :config
  (defvar my-org-light-frame nil)
  (defun my-org-html-export (orig-fun &rest args)
    (unless (and my-org-light-frame (frame-live-p my-org-light-frame))
      (setq my-org-light-frame (make-frame '((visibility . nil)
                                             (background-color . "White")))))
    (let ((buf (current-buffer))
          (orig-hide-fg (face-foreground 'org-hide nil t)))
      (unwind-protect
          (with-selected-frame my-org-light-frame
            (with-current-buffer buf
              (apply orig-fun args)))
        (set-face-foreground 'org-hide (or orig-hide-fg 'unspecified)))))
  (advice-add 'org-html-export-to-html :around #'my-org-html-export)
  (advice-add 'org-html-export-as-html :around #'my-org-html-export))
...
(use-package yasnippet
  :commands yas-expand corfu-yas-expand
  :config
  ...
  (defun corfu-yas-expand ()
    (interactive)
    (yas-minor-mode-on)
    (let ((completion-at-point-functions (list #'sk-yasnippet-capf)))
      (completion-at-point))))
...
(use-package yasnippet-capf
  ...
  :after yasnippet
  :config
  (defun sk-yasnippet-capf ()
    (when (thing-at-point-looking-at (rx (+ (or word punct)))
                                     yasnippet-capf-max-search-distance)
      (let ((table (completion-table-with-cache #'yasnippet-capf-candidates)))
        `(,(match-beginning 0) ,(match-end 0)
          ;; Reports the `yasnippet-key' completion category
          ,(completion-table-with-metadata table '((category . yasnippet-key)))
...
(use-package which-key
  ...
  :config
  (defun my-which-key-force-fit-window (&rest _)
    "Fit the which-key window to its buffer."
    (when-let* ((win (get-buffer-window which-key--buffer)))
      (dolist (w (window-list (window-frame win)))
        (when (window-preserved-size w)
          (window-preserve-size w nil nil)))
      (fit-window-to-buffer win nil 1)))
  (advice-add 'which-key--show-buffer-side-window
              :after #'my-which-key-force-fit-window))
...
(use-package stillness-mode
  :disabled
  :vc (:url "https://github.com/tttuuu888/stillness-mode.el"
            :rev :newest)
  :if (not (display-graphic-p))
  :defer-init (stillness-mode 1)
  :config
  (defvar sk-stillness-exclude-commands
    '(consult-git-grep
      consult-imenu consult-line consult-occur consult-rg-here
      consult-yank-from-kill-ring sk-consult-git-grep sk-consult-line
      sk-consult-yasnippet)
    "Commands `stillness-mode' should keep out of.")
  (define-advice stillness-mode--handle-point
      (:around (fn read-fn &rest args) sk-skip)
    (if (memq this-command sk-stillness-exclude-commands)
        (apply read-fn args)
      (apply fn read-fn args))))
...
(use-package vertico
  :bind (:map vertico-map
          ...
          ("C-a"      . my-vertico-beginning-of-line)
          ("C-j"      . ignore)    ;; consult-preview-key
          ("C-k"      . my-vertico-kill-line)
          ("C-t"      . vertico-multiform-vertical)
  ...
  (defun my-vertico-kill-line ()
    "Kill to the end of line, or clear the minibuffer if already at EOL.
Preserve the leading '#' if the input started with it."
    (interactive)
    (let ((has-hash (string-prefix-p "#" (minibuffer-contents))))
      (if (not (eolp))
          (kill-line)
        (delete-minibuffer-contents)
        (when has-hash (insert "#")))))
  (defun my-vertico-beginning-of-line ()
    "Move to the beginning of the line, or after '#' if present."
    (interactive)
    (let ((has-hash (string-prefix-p "#" (minibuffer-contents))))
      (move-beginning-of-line 1)
      (when has-hash (forward-char)))))
...
(use-package vertico-multiform
  ...
  :custom-face
  (+vertico-kill-ring-separator ((t :foreground "red")))
  :config
  (require 'dired)
  (require 'vertico-directory)
  (require 'vertico-sort)
  (setq
   vertico-side-style `(buffer
                        (vertico-group-format . nil)
                        (vertico-mode . t)
                        (vertico-posframe-mode . nil)
                        (lambda (_)
                          (setq-local vertico-count (/ (frame-height) 2))))
   +vertico-kill-ring-max-lines 6
   +vertico-transform-functions nil
   vertico-multiform-categories
   '((file (vertico-sort-function . +vertico-sort-dirs-and-files)
           (+vertico-transform-functions . +vertico-highlight-directory)
           (:keymap . vertico-directory-map))
     (t (vertico-sort-function . +vertico-default-sort)))
   vertico-multiform-commands
   `((consult-git-grep ,@vertico-side-style)
     (consult-imenu ,@vertico-side-style)
     (consult-line (vertico-mode . t) (vertico-posframe-mode . nil))
     (consult-occur ,@vertico-side-style)
     (consult-rg-here ,@vertico-side-style)
     (consult-yank-from-kill-ring ,@vertico-side-style)
     (sk-consult-git-grep ,@vertico-side-style)
     (sk-consult-line (vertico-mode . t) (vertico-posframe-mode . nil))
     (sk-consult-yasnippet (vertico-mode . t) (vertico-posframe-mode . nil)
                           (vertico-sort-function . +vertico-default-sort))))
  (cl-defmethod vertico--format-candidate :around
    (cand prefix suffix index start
          &context ((not +vertico-transform-functions) null))
  ...
  (defun +vertico-highlight-directory (file)
    ...
    (if (string-suffix-p "/" file)
        (let ((copy (copy-sequence file)))
          (add-face-text-property 0 (length copy) 'dired-directory 'append copy)
          copy)
      file))
  (defun +vertico-kill-ring--height (cand)
    "Calculate height of `kill-ring' candidate CAND."
    (let ((n (1+ (cl-count ?\n (string-trim-right cand "\n")))))
      (+ (min n +vertico-kill-ring-max-lines)
         (if (> n +vertico-kill-ring-max-lines) 2 1)))) ; "[...]" and separator
  (defun +vertico-kill-ring--trim (cand)
    "Return CAND without trailing newline, truncated with [...] if too long."
    (let* ((trimmed (string-trim-right cand "\n"))
           (lines (split-string trimmed "\n")))
      (if (<= (length lines) +vertico-kill-ring-max-lines)
          trimmed
        (concat (string-join (seq-take lines +vertico-kill-ring-max-lines) "\n")
                "\n[...]"))))
  (defun +vertico-kill-ring--avail ()
    "Calculate available lines to display candidates in window."
    (if-let* ((_ (overlayp vertico--candidates-ov))
              (win (overlay-get vertico--candidates-ov 'window))
              (_ (window-live-p win)))
        (max 1 (- (/ (window-pixel-height win) (default-line-height))
                  (if mode-line-format 2 1)))   ; minus prompt and mode-line
      (max 1 (/ (frame-height) 2))))
  (cl-defmethod vertico--arrange-candidates
    (&context ((vertico--metadata-get 'category) (eql kill-ring)))
    "Return a list of formatted, multiline `kill-ring' candidate strings."
    (let ((avail (+vertico-kill-ring--avail)) ; available lines
          (index (max 0 vertico--index))      ; selected candidate
          ;; current page - start index / used lines / used candidate count
          (start 0) (used 0) (count 0))
      ;; Pack candidates into pages and keep the one holding index.
      (cl-loop for cand in vertico--candidates for i from 0
               for h = (+vertico-kill-ring--height cand)
               do (when (and (> count 0) (> (+ used h) avail)) ; page is full
                    (when (> i index) (cl-return)) ; index was on that page
                    (setq start i used 0 count 0)) ; start a new page at i
               do (setq used (+ used h) count (1+ count)))
      (setq-local vertico-count count)  ; so that C-v/M-v works correctly
      ;; Render the current page including the current index
      (cl-loop for cand in (nthcdr start vertico--candidates)
               for i from start repeat (1+ count) ; +1 to fill the bottom gap
               for str = (+vertico-kill-ring--trim (vertico--hilit cand))
               collect (concat
                        (vertico--format-candidate str "" "" i start)
                        ;; No separator after the last candidate
                        (when (< i (1- vertico--total))
                          (propertize (concat (make-string 40 ?―) "\n")
                                      'face '+vertico-kill-ring-separator))))))
  (defun my-vertico-sort (input candidates)
    "Sort CANDIDATES into exact, prefix, and rest groups based on INPUT.
Candidates within each group are sorted alphabetically."
    (if (or (string-empty-p input)
            (eq (aref input 0) ?^))
        (vertico-sort-alpha candidates)
      (let ((first-word (or (car (split-string input)) ""))
            exact prefix rest)
        (dolist (cand candidates)
          (cond ((string-equal-ignore-case input cand) (push cand exact))
                ((string-prefix-p first-word cand t) (push cand prefix))
                (t (push cand rest))))
        (nconc (vertico-sort-alpha exact)
               (vertico-sort-alpha prefix)
               (vertico-sort-alpha rest)))))
  (defun +vertico-default-sort (candidates)
    "Sort CANDIDATES with `my-vertico-sort' unless the input is empty."
    (let ((input (minibuffer-contents-no-properties)))
      (if (string-empty-p input)
          candidates
        (my-vertico-sort input candidates))))
  (defun +vertico-sort-dirs-and-files (candidates)
    "Partition CANDIDATES into directories and files, then sort each group."
    (let ((input (file-name-nondirectory (minibuffer-contents-no-properties)))
          dirs files)
      (dolist (x candidates)
        (if (string-suffix-p "/" x)
            (push x dirs)
          (push x files)))
      (nconc (my-vertico-sort input (nreverse dirs))
             (my-vertico-sort input (nreverse files)))))
  (defun my-vertico-directory-tilde ()
    "Insert ~ and let `vertico-directory-tidy' clean the prompt."
    (interactive)
    (insert "~")
...
(use-package vertico-repeat
  ...
  (setq vertico-repeat-transformers
        (seq-difference vertico-repeat-transformers
                        (list #'vertico-repeat--filter-empty
                              #'vertico-repeat--remove-long)))
  (defun my-vertico-repeat-save-dir (session)
    "Save `default-directory' in SESSION for `vertico-repeat'."
    (when session
      (append session (list (cons 'default-directory default-directory)))))

  (defun my-vertico-repeat-restore-dir (orig session)
    "Restore `default-directory' from SESSION when repeating."
    (let ((default-directory (alist-get 'default-directory (cddr session)
                                        default-directory)))
      (funcall orig session)))

  (defun my-vertico-repeat-reselect (session)
    "Re-select the saved candidate once async candidates stream in.
Stop trying if the input changes before the candidate appears."
    (when-let* ((cand (seq-find #'stringp (cddr session)))
                (input (cadr session))
                (_ (not vertico--lock-candidate)))
      (cl-labels
          ((reselect ()
             (if (equal (minibuffer-contents-no-properties) input)
                 (when-let* ((idx (seq-position vertico--candidates cand)))
                   (remove-hook 'consult--completion-refresh-hook #'reselect t)
                   (vertico--goto idx)
                   (vertico--exhibit)
                   ;; Keep consult's buffer preview in sync
                   (when consult--preview-function
                     (funcall consult--preview-function)))
               ;; input edited -> give up
               (remove-hook 'consult--completion-refresh-hook #'reselect t))))
        ;; buffer-local, reset on each minibuffer entry
        (add-hook 'consult--completion-refresh-hook #'reselect t t))))

  (add-hook 'minibuffer-setup-hook #'vertico-repeat-save)
  (add-to-list 'vertico-repeat-transformers #'my-vertico-repeat-save-dir t)
  (advice-add 'vertico-repeat--run :around #'my-vertico-repeat-restore-dir)
  (advice-add 'vertico-repeat--restore :after #'my-vertico-repeat-reselect))

(use-package vertico-posframe
  :preface
  (defconst vertico-bg-light "#e6f2ff")
  (defconst vertico-bg-dark "#282a36")
  :custom-face
  (vertico-posframe
   ((((background light)) :background ,vertico-bg-light :foreground "black")
    (((background dark)) :background ,vertico-bg-dark :foreground "gray80")))
  (vertico-posframe-border
   ((t (:inherit vertico-posframe))))
  ...
  (vertico-posframe-border-4
   ((t (:inherit vertico-posframe))))
  (border
   ((((background light)) :foreground ,vertico-bg-light)
    (((background dark)) :foreground ,vertico-bg-dark)))
  :defer-init (vertico-posframe-mode 1)
  :config
  (setq vertico-posframe-border-width 20
        vertico-posframe-size-function
        (lambda (buffer) (plist-put (vertico-posframe-get-size buffer)
                                    :width (my-vertico-posframe-width))))
  (defun my-vertico-posframe-width ()
    (min (- (frame-width) 5) 140))
  (defun my-vertico-posframe-truncate-lines (&rest _)
    (setq vertico-posframe-truncate-lines
          (< (string-width
              (buffer-substring-no-properties (point-min) (point-max)))
             (- (my-vertico-posframe-width) 2))))
  (defun my-vertico-posframe-pad-tty (&rest _)
    (unless (display-graphic-p)
      (with-current-buffer vertico-posframe--buffer
        (when (frame-live-p posframe--frame)
          (set-window-margins (frame-root-window posframe--frame) 1 1)))))
  (advice-add 'vertico-posframe--show :before
              #'my-vertico-posframe-truncate-lines)
  (advice-add 'vertico-posframe--show :after #'my-vertico-posframe-pad-tty))
...
(use-package consult
  ...
  (evil-leader/set-key
    ...
    "jp"    'consult-rg-here
    "nx"    'consult-org-capture
    "xri"   'consult-register-store
    "xrj"   'consult-register-load)
  :defer-init (require 'consult)
  :custom-face
  (consult-highlight-match
   ((((background light)) :foreground "#b00000" :extend t :inherit nil)
    (((background dark)) :foreground "gold1" :extend t :inherit nil)))
  :config
  (require 'smex)
  (require 'wgrep)
  (setq sk-compile-history nil
        ...
        (concat consult-ripgrep-args
                " --no-ignore-parent --max-columns-preview")
  ...
  (defun consult-org-capture ()
    "Capture something with consult completion."
    (interactive)
    (require 'org-capture)
    (let* ((templates
            (let (prefixes)
              (cl-mapcan
               ...
               (or (org-contextualize-keys
                   ...
                   '(("t" "Task" entry (file+headline "" "Tasks")
                      "* TODO %?\n  %u\n  %a"))))))
           (choice (consult--read templates
                                  :prompt "Capture template: "
                                  :require-match t)))
      (org-capture nil (car (split-string choice)))))
  ...
  (defun consult-rg-here ()
    (interactive)
    (consult-ripgrep default-directory nil))

  (defun sk-compile-scripts ()
    (let* ((target (or (locate-dominating-file default-directory ".sbin")
                       (user-error "Couldn't find .sbin folder")))
           (sbin (expand-file-name ".sbin" target))
           (files (or (directory-files sbin t "\\.sh$")
                      (user-error "No compile scripts in .sbin folder")))
           (files-alist
            (mapcar (lambda (f) (cons (file-name-nondirectory f) f)) files)))
      (cons (file-name-as-directory sbin) files-alist)))

  (defun sk-compile (&optional dir)
    (interactive)
    (let* ((result (sk-compile-scripts))
           (sbin (car result))
           (scripts (cdr result))
           (last-cmd (alist-get sbin sk-compile-history nil nil #'equal))
           (default (when last-cmd (file-name-nondirectory last-cmd)))
           (script (consult--read
                    (mapcar #'car scripts)
  ...
  (defun sk-consult-line--preselect (curr-line)
    "Select the first candidate at or after CURR-LINE.
The selection happens once vertico has computed its candidates."
    (cl-labels
        ((preselect ()
           (when (consp vertico--candidates)
             (remove-hook 'post-command-hook #'preselect t)
             ;; Don't override a candidate restored by `vertico-repeat',
             ;; which locks the previously selected candidate.
             (unless vertico--lock-candidate
               ;; Select the first candidate at or after point
               (when-let* ((idx (seq-position
                                 vertico--candidates curr-line
                                 (lambda (cand target)
                                   (>= (cdr (get-text-property
                                             0 'consult-location cand))
                                       target)))))
                 (vertico--goto idx)
                 (vertico--exhibit)
                 ;; Refresh consult's preview for the new selection
                 (when consult--preview-function
                   (funcall consult--preview-function)))))))
      ;; buffer-local, reset on each minibuffer entry
      (add-hook 'post-command-hook #'preselect 90 t)))

  (defun sk-consult-line (&optional initial)
    (interactive)
    (let ((curr-line (line-number-at-pos (point) consult-line-numbers-widen))
          (symbol (thing-at-point 'symbol)))
      (minibuffer-with-setup-hook
          (lambda ()
            (when symbol (setq minibuffer-default symbol))
            (sk-consult-line--preselect curr-line))
        (consult-line initial (not consult-line-start-from-top)))))
  ...
  (consult-customize consult-occur :preview-key 'any)
  (consult-customize consult-recent-file :category 'recent-file)
  (consult-customize sk-consult-line :preview-key 'any))
...
(use-package consult-yasnippet
  :commands sk-consult-yasnippet
  :init
  (evil-leader/set-key "/" 'sk-consult-yasnippet)
  :config
  (defun sk-consult-yasnippet (arg)
    (interactive "P")
    (yas-minor-mode-on)
...
(use-package embark
  ...
  (setq embark-cycle-key "M-o"
        ...
        (assq-delete-all 'project-file embark-transformer-alist)
  ...
  (defun my-create-dir-locals (path)
    (find-file (expand-file-name ".dir-locals.el" path)))

  (defvar-keymap embark-project-actions
    :doc "Keymap for Embark project actions."
    :parent embark-general-map
    "d" #'sk-remove-known-project)
  (add-to-list 'embark-keymap-alist '(project-file . embark-project-actions))

  (defvar-keymap embark-recent-file-map
    :doc "Keymap for Embark actions on consult-recent-file."
    :parent embark-file-map
    "d" #'embark-recentf-remove
    "D" #'delete-file)
  (add-to-list 'embark-keymap-alist '(recent-file . embark-recent-file-map))

  (defvar-keymap embark-my-file-actions
    :doc "Keymap for Embark file actions."
    :parent embark-file-map
    "." #'my-create-dir-locals
    "p" #'sk-create-project)
  (add-to-list 'embark-keymap-alist '(file . embark-my-file-actions))
  (advice-add 'embark-kill-ring-remove :before
              (lambda (text)
                (when (equal text (car kill-ring-yank-pointer))
                  (setq kill-ring-yank-pointer
                        (cdr kill-ring-yank-pointer))))))
...
(use-package popper
  ...
  (defun my-popper-display-popup (buffer &optional alist)
    "Display popup BUFFER at the bottom.
Select its window unless BUFFER matches `my-popper-no-focus-buffers'."
    (let ((name (buffer-name (get-buffer buffer))))
      (if (seq-some (lambda (re) (string-match-p re name))
                    my-popper-no-focus-buffers)
...
(use-package elec-pair
  :ensure nil
  :defer-init (electric-pair-mode 1)
  :config
  (defun electric-pair-delete-pair (arg &optional killp)
    "Custom pair-delete.
Delete a closing braket in case of (|), delete a pair of inner braket
in case of ((|))."
    (interactive "*p\nP")
...
(use-package which-func
  ...
  (add-hook 'lisp-interaction-mode-hook
            (lambda () (setq-local which-func-mode nil)))
  :defer-init (which-function-mode 1)
  :config
...
(use-package project
  ...
  :commands (sk-add-known-project
             ...
             sk-project-kill-buffers
             sk-project-save-buffers)
  :init
  (evil-leader/set-key
    "jk" 'sk-project-kill-buffers
    "jb" 'project-switch-to-buffer
    "js" 'project-switch-project
    "jS" 'sk-project-save-buffers)
  :config
  ;; Only remember projects added by `sk-add-known-project'. This blocks both
  ;; `project-current' and `project-switch-project', which remember silently.
  (defvar sk-project-remember nil
    "Non-nil only while adding a project.")
  (setq project-find-functions '(sk-project-try)
        project-list-exclude (list (lambda (_) (not sk-project-remember)))
        project-prompter #'sk-project-prompt-project-dir
        project-switch-commands 'project-dired)
  (defun sk-project-try (dir)
    "Return the project of DIR, rooted at a `sk-project-root-files' marker."
    (when-let* ((root (sk-project-root dir)))
      (cons 'sk root)))
  (defun sk-project-prompt-project-dir (&optional prompt &rest _args)
    "Prompt for one of the known project roots, except the current one."
    (project--ensure-read-project-list)
    (unless (project-known-project-roots)
      (user-error "There are no known projects"))
    (let* ((root (sk-project-root))
           (roots (and root (list root (abbreviate-file-name root))))
           (pred (lambda (cand)
                   (not (member (if (consp cand) (car cand) cand) roots)))))
      (project-prompt-project-dir prompt pred t)))
  (cl-defmethod project-root ((project (head sk)))
    (cdr project))
  (defun sk-project-save-buffers ()
    "Save all modified buffers of the current project."
    (interactive)
    (project-save-some-buffers t))
  (defun sk-add-known-project (project-root)
    "Make .project file and add the project to known projects list."
    (interactive (list (read-directory-name "Add to known projects: ")))
    (sk-create-project project-root)
    (let ((sk-project-remember t))
      (project-remember-project (sk-project-try project-root))))
  (defun sk-remove-known-project (project)
    "Show the current project at the top of the candidate list."
    (interactive
     (let* ((root (sk-project-root))
            (root (and root (abbreviate-file-name root)))
            (cands (project-known-project-roots))
            (cands (if (and root (member root cands))
                       (cons root (remove root cands))
                     cands)))
       (list (completing-read "Remove from known projects: " cands nil t))))
    (project-forget-project project))
  (defun sk-project-kill-buffers (&optional no-confirm)
    "Kill the buffers belonging to the current project.
Outside a project, kill the buffers under `default-directory'."
    (interactive)
...
(use-package eglot
  ...
  (setq eglot-autoshutdown t
        ...
        eglot-server-programs (append `((python-mode . ("ty" "server")))
...
(use-package irony
  :hook ((c++-mode c-mode objc-mode) . sk-irony-mode)
  :config
  (defun my-c-corfu-trigger ()
    "Auto-start completion after C/C++ member-access operators."
    (when (and (bound-and-true-p corfu-mode)
               (memq last-command-event '(?> ?: ?.))
               (not (nth 8 (syntax-ppss))) ; not in string/comment
               (looking-back "\\(?:->\\|::\\|\\.\\)" (- (point) 2)))
      (ignore-errors (completion-at-point))))
  (advice-add 'company-irony-c-headers--candidates :filter-args
              (lambda (args)
                (let (p)
                  (if (and (stringp (car args))
                           (not (get-text-property 0 'quote (car args)))
                           (setq p (company-irony-c-headers--prefix)))
                      (list (propertize (car args) 'quote
                                        (get-text-property
                                         0 'quote (if (consp p) (car p) p))))
                    args))))
  (defun my-irony-corfu-setup ()
    (add-hook 'completion-at-point-functions
              (cape-company-to-capf #'company-irony) nil t)
    (add-hook 'completion-at-point-functions
              (cape-company-to-capf #'company-irony-c-headers) nil t)
    (add-hook 'post-self-insert-hook 'my-c-corfu-trigger nil t))
  (defun sk-irony-mode ()
    (unless (file-remote-p default-directory)
      (irony-mode)))
  (add-hook 'irony-mode-hook 'irony-cdb-autosetup-compile-options)
  (add-hook 'irony-mode-hook 'my-irony-corfu-setup)
...
(use-package web-mode
  ...
  (defun my-web-corfu-setup ()
    (add-hook 'completion-at-point-functions
              (cape-company-to-capf #'company-web-html) nil t))
  (add-hook 'web-mode-hook 'my-web-corfu-setup))
...
(use-package cmake-mode
  ...
  (defun my-cmake-corfu-setup ()
    (add-hook 'completion-at-point-functions
              (cape-company-to-capf #'company-cmake) nil t))
  (add-hook 'cmake-mode-hook 'my-cmake-corfu-setup))
...
(use-package dumb-jump
  :config
  (setq dumb-jump-project-denoters sk-project-root-files)
