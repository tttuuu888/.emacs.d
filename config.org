  ...
  (use-package dired
      ...
      (let* ((defaults (dired-dwim-target-defaults nil nil))
              ...
              (expand-file-name
               (minibuffer-with-setup-hook
                 ...
                 (dired-mark-read-file-name
                  "Rsync to: " default-directory 'copy nil files nil))))
             (src-remote-p (file-remote-p default-directory))
             (dst-remote-p (file-remote-p dest))
             ...
             (cmd (concat "rsync -ahs"
                          ...
                          (mapconcat
                           ...
                           files " ")
                          " \"" prefix "\""))
             (default-directory (if src-remote-p "~/" default-directory)))
        ...
        (if (and src-remote-p dst-remote-p)
            (user-error "Remote to remote is not supported.")
      ...
      (let* ((defaults (dired-dwim-target-defaults nil nil))
        ...
        (when ext
          (async-shell-command-callback
           (concat cmd name " " (string-join files " "))
           (lambda (file)
    ...
    (defun my-dired-do-copy ()
      ...
      (let* ((defaults (dired-dwim-target-defaults nil nil))
              ...
              (expand-file-name
               (minibuffer-with-setup-hook
                   (lambda ()
                     ...
                     (setq minibuffer-default defaults))
                 (dired-mark-read-file-name
                  "Copy to: " default-directory 'copy nil files nil))))
        ...
        (if (not remote)
            (async-shell-command-callback
             (concat "cp -rf " (string-join files " ") " " target)
             (lambda (target-files)
               (mapc
    ...
    (defun my-dired-do-delete ()
      ...
      (let* ((remote (file-remote-p default-directory))
        ...
        (when (and files
          ...
          (if (not remote)
              (async-shell-command-callback
               (concat "rm -rf " (string-join files " "))
               (lambda (files)
