...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package inf-elixir
    :commands (run-iex inf-elixir-send-line-or-region
                       inf-elixir-reload-or-load-module)
    :config
    (defun run-iex ()
      (interactive)
      ...
      (if (region-active-p)
        ...
        (inf-elixir--send
         (buffer-substring
          (line-beginning-position) (line-end-position n))))
      (setq deactivate-mark t))
    (defun inf-elixir-reload-or-load-module ()
      "Reloads the module if it's already loaded, or compiles and loads it."
      (interactive)
      (if-let* ((buffer (inf-elixir--determine-repl-buf))
                (module (nth 0 (inf-elixir--matches-in-buffer
                                "defmodule \\([A-Z][A-Za-z0-9\._]+\\)\s+")))
                (check-module (format ":code.is_loaded %s\n" module))
                (compile-file
                 (replace-regexp-in-string
                  (rx "\\" (group anything))
                  "\\1"
          ...
          (with-current-buffer buffer
            (comint-send-string buffer check-module)
            ;; Can be replaced with a function like `comint-wait-for-output'.
            (sleep-for 0.1)
            (when (equal "false"
                         (save-excursion
  ...
  (use-package elixir-mode
      ...
      (let* ((target-dir (expand-file-name "elixir-ls/" user-emacs-directory))
        ...
        (if (and (file-exists-p ver-file)
          ...
          (save-window-excursion
            ...
            (download-github-release-package owner repo ver "elixir-ls.zip" zip)
    ...
    (evil-leader/set-key-for-mode 'elixir-mode
      ...
      "em" 'inf-elixir-reload-or-load-module
