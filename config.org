      ...
      (let ((sort-func
             (lambda (a b)
               (if (string-empty-p input)
                   ;; Default sort when no input is provided
                   (string-lessp a b)
                 (let ((a-prefix (string-prefix-p input a t))
                       (b-prefix (string-prefix-p input b t)))
                   (cond
                    ;; Both match the prefix: prioritize the shorter one
                    ((and a-prefix b-prefix)
                     (let ((len-a (length a))
                       ...
                       (if (/= len-a len-b)
                           (< len-a len-b)
                         (string-lessp a b))))
                    ;; Only one matches the prefix: prioritize that one
                    (a-prefix t)
                    (b-prefix nil)
                    ;; Neither matches the prefix: fall back to default sort
                    (t (string-lessp a b))))))))
        (sort (copy-sequence candidates) sort-func)))
