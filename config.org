...
#+BEGIN_SRC emacs-lisp :tangle yes
  (when (member "-init" command-line-args)
    ...
    (require 'pinstall "~/.emacs.d/pinstall.el")
    (pinstall-init))
#+END_SRC
...
At this point, =user-emacs-directory= is set in order to =~/.emacs.d/.local/= to
hide all package local files.  From this point, =user-emacs-directory= is set to
=~/.emacs.d/.local/= so that all package local files would be hidden under the
path.
...
Stop Emacs default functionality saving installed packages list in =custom.el= file.
