...
(use-package vertico-multiform
  ...
  (defun +vertico-kill-ring--avail ()
    ...
    (if-let* (((overlayp vertico--candidates-ov))
  ...
  (defun my-vertico-directory-tilde ()
    ...
    (if (eq (char-before) ?/)
        (progn
          ...
          (insert "~/"))
      (insert "~")))
  (defun my-vertico-directory-slash ()
    "Insert / and clean the prompt."
    (interactive)
    (if (eq (char-before) ?/)
        (progn
...
(use-package vertico-repeat
  ...
  (defun my-vertico-repeat-restore-dir (orig session)
    ...
    (let* ((entry (seq-find (lambda (x) (and (consp x)
                                             (eq (car x) 'default-directory)))
                            (cddr session)))
           (default-directory (if entry (cdr entry) default-directory)))
            ...
            (lambda ()
              (if (equal (minibuffer-contents-no-properties) input)
                  (when-let* ((idx (seq-position vertico--candidates cand)))
                    (setq vertico--index idx vertico--lock-candidate t)
...
(use-package vertico-posframe
  ...
  :custom-face
  (vertico-posframe
   ((((background light)) :background "#e6f2ff" :foreground "black")
          ...
          (lambda ()
            ...
            (let ((done nil))
              (add-hook
               ...
               (lambda ()
                 ;; Run only once
                 (when (and (not done) (consp vertico--candidates))
                   (setq done t)
                   ...
                   (unless vertico--lock-candidate
                     ;; Select the candidate on the current line if present,
                     ;; otherwise keep vertico's default (first match)
                     (when-let* ((idx (seq-position
                       ...
                       (when consult--preview-function
                         (funcall consult--preview-function))))))
               90 t)))    ;; buffer-local, reset on each minibuffer entry
