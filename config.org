...
#+BEGIN_SRC emacs-lisp
  ...
  If RETURN-ONLY-DIR is non-nil, return the directory containing the
  file instead of the full path to the file itself."
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package magit
    ...
    (advice-add 'magit-diff-visit-file :around #'my-magit-diff-visit)
    ...
    (defun sk-save-git-diff (diff-file)
      ...
      (if (not (magit-staged-files))
        ...
        (call-process "git" nil `(:file ,diff-file) nil
                      "--no-pager" "diff" "--cached")))
    (defun my-magit-diff-visit (orig-fun &rest args)
      "Visit worktree file unless the current line starts with '-'."
      (let* ((line-text (buffer-substring-no-properties
                         (line-beginning-position)
                         (line-end-position)))
             (first-char (if (> (length line-text) 0)
