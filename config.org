  ...
  (defun my-vertico-sort (input candidates)
    (if (string-empty-p input)
      ...
      (let* ((trimmed (string-trim input))
             (first-word (car (split-string trimmed " ")))
             res-prefix res-others)
        (dolist (cand candidates)
          (if (string-prefix-p first-word cand t)
              (push cand res-prefix)
            (push cand res-others)))
        (nconc
         (sort (nreverse res-prefix)
               (lambda (a b) (string-collate-lessp a b nil t)))
         (sort (nreverse res-others)
               (lambda (a b) (string-collate-lessp a b nil t)))))))
  (defun vertico-default-sort (candidates)
    "Sort candidates with `my-vertico-sort', or return as-is if input is empty."
    ...
    (let ((input (file-name-nondirectory (minibuffer-contents-no-properties)))
      ...
      (dolist (x lst)
        (if (string-suffix-p "/" x)
            (push x dirs)
          (push x files)))
      (nconc (my-vertico-sort input dirs)
             (my-vertico-sort input files))))
