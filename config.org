...
(use-package helm-ag
  ...
  :init
  (evil-leader/set-key "jp" 'helm-ag-here)
  :config
  (require 'wgrep)
  ...
  (advice-add 'helm-ag--persistent-action :after
              (lambda (&rest _) (which-func-update)))
  (defun helm-ag-here ()
    "Run helm-ag in `default-directory', or on marked files when in dired."
    (interactive)
    (let ((files (and (derived-mode-p 'dired-mode)
      ...
      (helm-do-ag default-directory
                  files
                  (if files "" (thing-at-point 'symbol))))))
...
(use-package vertico-multiform
  ...
  (setq
   ...
   '((consult-line (vertico-posframe-mode . nil) (vertico-mode . t))
     (consult-rg-here (vertico-posframe-mode . nil) (vertico-mode . t))
...
(use-package vertico-posframe
  ...
  (vertico-posframe-border-4
   ((t (:inherit vertico-posframe))) face-defface-spec)
  :bind (:map vertico-map
          ("C-t" . my-vertico-toggle-posframe))
  :defer-init (vertico-posframe-mode 1)
  :config
  (setq vertico-posframe-border-width 20)
  (defvar-local my-vertico-posframe--inhibit nil)
  (defun my-vertico-toggle-posframe ()
    "Toggle posframe display for the current vertico session only."
    (interactive)
    (setq-local my-vertico-posframe--inhibit
                (not my-vertico-posframe--inhibit))
    (let ((win (active-minibuffer-window)))
      (when my-vertico-posframe--inhibit
        (posframe-hide vertico-posframe--buffer)
        (set-window-vscroll win 0)
        (setq-local max-mini-window-height 1.0)))
    (vertico--exhibit))
  (define-advice vertico-posframe-mode-workable-p
      (:around (fn) respect-inhibit)
    (and (not my-vertico-posframe--inhibit) (funcall fn)))
...
(use-package consult
  ...
  (defun consult-rg-here (&optional initial)
    "Search with ripgrep in `default-directory', including VCS-ignored files."
    (interactive "P")
    (let ((consult-ripgrep-args
           (concat consult-ripgrep-args
                   " --no-ignore-vcs --glob-case-insensitive"))
          (vertico-count (/ (frame-height) 2))
          (vertico-group-format nil))
      (consult--grep "Ripgrep" #'consult--ripgrep-make-builder
