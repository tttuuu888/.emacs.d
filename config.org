...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-find-file-in-git-project (regex)
    (when (zerop (call-process "git" nil nil nil "-C" "." "rev-parse"))
      (let* ((root (shell-command-to-string "git rev-parse --show-toplevel"))
             (default-directory (substring root 0 (1- (length root))))
  ...
  (defun sk-find-other-file (&optional FLEX-MATCHING)
    "Switch betwen .c|cpp and .h files. It only works for git projects."
    (interactive)
    (if-let* ((file (buffer-file-name))
              (ext (when file (file-name-extension file)))
              (dir (file-name-directory file))
              (root (shell-command-to-string "git rev-parse --show-toplevel"))
              (root (substring root 0 (1- (length root))))
              (regex (if (equal ext "h")
                         (concat (file-name-base file) ".(c|cpp)$")
                       (concat (file-name-base file) ".h$")))
              (others (my-find-file-in-git-project regex)))
        (if (= (length others) 1)
            (find-file (expand-file-name (car others) root))
          (ivy-read "Switch to: " others
                    :action (lambda (file)
                             (find-file (expand-file-name file root)))
                    :caller #'sk-find-other-file))))
#+END_SRC
