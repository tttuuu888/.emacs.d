  ...
  (use-package dired
    ...
    (defun my-dired-compress ()
      ...
      (let* ((defaults (dired-dwim-target-defaults nil nil))
             ...
             (inital-name (file-name-nondirectory
                           (if (file-directory-p (dired-get-filename nil t))
        ...
        (pcase ext
          (`"zip" (setq cmd "zip -r ")))
        (when ext
          (async-shell-command (concat cmd name " " (string-join files " "))))))
    (defun my-dired-do-copy ()
      ...
      (let* ((defaults (dired-dwim-target-defaults nil nil))
             ...
             (cmd (if (string-prefix-p "/ssh" target) "scp -r " "cp -r "))
             (target-files
              (mapcar
               (lambda (f) (expand-file-name (file-name-nondirectory f) target))
               files))
             (target (string-remove-prefix "/ssh:" target)))
        (async-shell-command-callback
         (concat cmd (string-join files " ") " " target)
         (lambda (target-files)
