...
#+begin_src emacs-lisp
  ...
  (defun use-package-handler/:defer-init (name _keyword arg rest state)
    "Defer code execution until the first command after initialization."
    (use-package-concat
     (let ((init-body
            (use-package-hook-injector (use-package-as-string name)
                                       :defer-init arg)))
       (when init-body
         (let ((name-sym (gensym "defer-init-")))
           `((defun ,name-sym ()
               ,@init-body
               (remove-hook 'pre-command-hook ',name-sym)
               (unintern ',name-sym nil))
             (add-hook 'pre-command-hook #',name-sym)))))
